...
- again refresh webpage(http://3.134.112.139:8080), yes now shon jenkins dashboard
...
## next launch one ec2-instance & install docker & pull image(image_from_container) from your docker-hub-repo, see:
- create one ec2-instance with amazon-linux
- ssh that instance & install docker & start docker



## pull image from docker-hub-repo & create container from this image & after creating container go inside & see all files which you created before uploading/push
```bash
# see images
...
# now that image is private so you can pull after defin credential, see:
