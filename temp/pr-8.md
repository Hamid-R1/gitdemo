...
## Step-F: create bastion-server in public-subnet-1
- Name:  bastion-server
- ami:  amazon-linux-2
- Instance type:  t2.micro
- Key pair:  wp-project (select existing one)
...
- ssh `bastion-server` & copy `wp-project.pem` file from local pc to this `bastion-server`
- ssh -i "wp-project.pem" ec2-user@private-ip-of-app-server-1
	- deployment
	```bash
	vim deploy-wp.sh
	cat deploy-wp.sh
	...
	DBEndpoint='hr-rds-db.ciqr1l62rych.ap-southeast-1.rds.amazonaws.com'
	```
- ssh -i "wp-project.pem" ec2-user@private-ip-of-app-server-2
